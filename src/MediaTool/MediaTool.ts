...
export class MediaTool {
     ...
     * @returns 
     */
    async convert(from: MediaType, to: MediaType, sourceFile: MediaToolSource): Promise<Blob> {
     ...
     * @returns 
     */
    async addOverlay(source: MediaToolSource, overlay: MediaToolSource, overlayRect: MediaToolOverlayRect): Promise<Blob> {
        ...
        await this.ffmpeg.run('-re', '-i' ,'source.mp4', '-vf', `movie=overlay.mp4, scale=${overlayRect.width}x${overlayRect.height}[test]; [in][test]overlay=x=${overlayRect.x}:y=${overlayRect.y}  [out]`, '-vcodec', 'libx264', 'output.mp4');
     ...
     */
    async videoAppendAudio(video: MediaToolSource, audios: MediaToolAudioInsert[]): Promise<Blob> {
        ...
        for (let index = 1; index < audios.length; index++) {
            ...
            fileterString +=  `[${index}]` + 'adelay=' + `${delay * 1000} | ${delay * 1000}` + `[s${index}]` + ';';
        }
